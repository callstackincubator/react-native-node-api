// Ported from Node.js' test/node-api/test_threadsafe_function/binding.c.
// Upstream uses libuv's threading library; React Native has no libuv, so a
// small pthread-based shim stands in for the uv_* calls. The test logic and
// its assertions are kept as close to upstream as practical, with supplements
// marked as such: the call-into-JS callbacks assert they run on the JS thread,
// and Ref is exported alongside Unref (upstream exercises unref through a
// child-process teardown test, which does not port to React Native).
#include <node_api.h>
#include <pthread.h>
#include <stdbool.h>
#include <stdint.h>
...
// Upstream uses ARRAY_LENGTH 10000 and pauses every 1000 items; scaled down
// to keep the on-device runtime within the test timeout while preserving the
// ratios that matter: ARRAY_LENGTH / 2 must exceed Hermes' 1000-item tsfn
// dispatch budget (kMaxDispatchCount in API/napi/hermes_napi_tsfn.cpp) so the
// final run exercises the budget-exhausted re-post path, and the abort runs
// still get multiple pause windows.
#define ARRAY_LENGTH 2500
#define MAX_QUEUE_SIZE 2
#define PAUSE_EVERY 250

// pthread-based stand-ins for the libuv threading APIs used upstream.
...
static void* uv_thread_shim_main(void* arg) {
  uv_thread_shim shim = *(uv_thread_shim*)arg;
  free(arg);
  shim.entry(shim.arg);
  return NULL;
}
...
static int uv_thread_create(uv_thread_t* tid, uv_thread_cb entry, void* arg) {
  uv_thread_shim* shim = malloc(sizeof(uv_thread_shim));
  if (shim == NULL) return -1;
  shim->entry = entry;
  shim->arg = arg;
  int result = pthread_create(tid, NULL, uv_thread_shim_main, shim);
  if (result != 0) free(shim);
  return result;
}

static int uv_thread_join(uv_thread_t* tid) { return pthread_join(*tid, NULL); }

static uint64_t uv_hrtime(void) {
  struct timespec ts;
  clock_gettime(CLOCK_MONOTONIC, &ts);
  return (uint64_t)ts.tv_sec * 1000000000u + (uint64_t)ts.tv_nsec;
}

// Supplement: the thread the addon was initialized on, i.e. the JS thread.
static pthread_t js_thread;

static void assert_on_js_thread(const char* who) {
  if (!pthread_equal(pthread_self(), js_thread)) {
    napi_fatal_error(who, NAPI_AUTO_LENGTH,
        "expected to be called on the JS thread", NAPI_AUTO_LENGTH);
  }
}
...
static void data_source_thread(void* data) {
  ...
  for (index = ARRAY_LENGTH - 1; index > -1 && !queue_was_closing; index--) {
    ...
    if (ts_fn_info->max_queue_size == 0 && (index % PAUSE_EVERY == 0)) {
...
static void call_js(napi_env env, napi_value cb, void* hint, void* data) {
  if (!(env == NULL || cb == NULL)) {
    ...
    NODE_API_CALL_RETURN_VOID(
        env, napi_call_function(env, undefined, cb, 1, &argv, NULL));
  }
}
...
static napi_value StopThread(napi_env env, napi_callback_info info) {
  ...
  NODE_API_CALL(env,
      napi_release_threadsafe_function(
          ts_fn, abort ? napi_tsfn_abort : napi_tsfn_release));
  ts_fn = NULL;
  return NULL;
}
...
static void join_the_threads(napi_env env, void* data, void* hint) {
  assert_on_js_thread("join_the_threads");
  ...
  NODE_API_CALL_RETURN_VOID(
      env, napi_get_reference_value(env, the_hint->js_finalize_cb, &js_cb));
  NODE_API_CALL_RETURN_VOID(env, napi_get_undefined(env, &undefined));
  NODE_API_CALL_RETURN_VOID(
      env, napi_call_function(env, undefined, js_cb, 0, NULL, NULL));
  NODE_API_CALL_RETURN_VOID(
      env, napi_delete_reference(env, the_hint->js_finalize_cb));
  if (alt_ref != NULL) {
...
static napi_value StartThreadInternal(napi_env env, napi_callback_info info,
    napi_threadsafe_function_call_js cb, bool block_on_full,
    bool alt_ref_js_cb) {
  size_t argc = 4;
  napi_value argv[4];
  ...
  NODE_API_CALL(env,
      napi_create_string_utf8(env, "Node-API Thread-safe Function Test",
...
static napi_value Ref(napi_env env, napi_callback_info info) {
  ...
  NODE_API_CALL(env, napi_ref_threadsafe_function(env, ts_fn));
  return NULL;
}
...
static napi_value Release(napi_env env, napi_callback_info info) {
  NODE_API_ASSERT(env, ts_fn != NULL, "No existing thread-safe function");
  NODE_API_CALL(
      env, napi_release_threadsafe_function(ts_fn, napi_tsfn_release));
  return NULL;
}
...
static void ThreadSafeFunctionFinalize(napi_env env, void* finalize_data,
    void* finalize_hint) {
  assert_on_js_thread("ThreadSafeFunctionFinalize");
...
static napi_value CallIntoModule(napi_env env, napi_callback_info info) {
  ...
  NODE_API_CALL(env,
      napi_create_threadsafe_function(env, argv[0], argv[1], argv[2], 0, 1,
          finalize_func, ThreadSafeFunctionFinalize, NULL, NULL, &tsfn));
  NODE_API_CALL(
      env, napi_call_threadsafe_function(tsfn, NULL, napi_tsfn_blocking));
