...
static int uv_thread_create(uv_thread_t* tid, uv_thread_cb entry, void* arg) {
  try {
    tid->t = std::make_unique<std::thread>([entry, arg]() { entry(arg); });
    return 0;
  } catch (...) {
    return -1;
  }
}
static void uv_thread_join(uv_thread_t* tid) {
  // printf("Thread is joinable: %d %d\n", tid->t->joinable(),
  //  std::this_thread::get_id());
  if (tid->t && tid->t->joinable()) {
    // printf("Joining thread: %d\n", std::this_thread::get_id());
    tid->t->join();
  }
}
static size_t uv_hrtime() {
...
static void secondary_thread(void* data) {
  // printf("Secondary thread started: %d\n", std::this_thread::get_id());
...
static void data_source_thread(void* data) {
  // printf("Data source thread started: %d\n", std::this_thread::get_id());
  ...
  if (ts_fn_info->start_secondary) {
    ...
    // printf("Creating secondary thread: %d\n", std::this_thread::get_id());
...
static void call_js(napi_env env, napi_value cb, void* hint, void* data) {
  if (!(env == NULL || cb == NULL)) {
    ...
    // printf("Calling JS function (%p, %p) from thread: %d\n", env, cb,
    //  std::this_thread::get_id());
...
static napi_value StopThread(napi_env env, napi_callback_info info) {
  ...
  NODE_API_CALL(
      env, napi_create_reference(env, argv[0], 1, &(ts_info.js_finalize_cb)));
  bool abort;
  NODE_API_CALL(env, napi_get_value_bool(env, argv[1], &abort));
  // printf("[C++] Stopping thread-safe function...\n");
  NODE_API_CALL(env, napi_release_threadsafe_function(
...
static void join_the_threads(napi_env env, void* data, void* hint) {
  ...
  uv_thread_t* the_threads = (uv_thread_t*)data;
  ts_fn_hint* the_hint = (ts_fn_hint*)hint;
...
static napi_value StartThreadInternal(napi_env env, napi_callback_info info,
                                      ...
                                      bool block_on_full, bool alt_ref_js_cb) {
  size_t argc = 4;
  napi_value argv[4];

  // printf("Starting thread-safe function from thread: %d\n",
  //  std::this_thread::get_id());
  ...
  NODE_API_CALL(env,
                napi_create_string_utf8(env, "N-API Thread-safe Function Test",
  ...
  NODE_API_CALL(env,
                napi_create_threadsafe_function(
                    env,                     // napi_env env,
                    argv[0],                 // napi_value func,
                    NULL,                    // napi_value async_resource,
                    async_name,              // napi_value async_resource_name,
                    ts_info.max_queue_size,  // size_t max_queue_size,
                    2,                       // size_t initial_thread_count,
                    uv_threads,              // void* thread_finalize_data,
                    join_the_threads,  // napi_finalize thread_finalize_cb,
                    &ts_info,          // void* context,
                    cb,        // napi_threadsafe_function_call_js call_js_cb,
                    &ts_fn));  // napi_threadsafe_function* result
  bool abort;
  NODE_API_CALL(env, napi_get_value_bool(env, argv[1], &abort));
  ...
  NODE_API_ASSERT(
      env, (uv_thread_create(&uv_threads[0], data_source_thread, ts_fn) == 0),
      "Thread creation");
...
static napi_value CallIntoModule(napi_env env, napi_callback_info info) {
  ...
  NODE_API_CALL(env, napi_create_threadsafe_function(
                         env, argv[0], argv[1], argv[2], 0, 1, finalize_func,
